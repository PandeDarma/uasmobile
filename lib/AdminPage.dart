...
class _AdminPageState extends State<AdminPage> {
  ...
  Future<List> getData() async {
    final response =
        await http.get("http://192.168.43.80:8080/api_pariwisata/getdata.php");
  ...
  Widget build(BuildContext context) {
    return new Scaffold(
      appBar: new AppBar(
        title: new Text("List Booking"),
        actions: <Widget>[
          IconButton(
          icon:  Icon(Icons.arrow_back), 
          onPressed:  ()  {Navigator.pushReplacementNamed(context, '/MyHomePage');
          },
          ),
        ],
      ),
